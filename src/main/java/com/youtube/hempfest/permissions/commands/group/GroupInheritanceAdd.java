...
public class GroupInheritanceAdd extends BukkitCommand {


    public GroupInheritanceAdd(String name, String description, String permission, String usageMessage, ArrayList<String> aliases) {
    ...
    public boolean execute(CommandSender commandSender, String commandLabel, String[] args) {
        ...
        if (!(commandSender instanceof Player)) {
            ...
            if (length == 0) {
                sendMessage(commandSender, um.prefix + "&c&o/" + commandLabel + " <groupName> <worldName> <groupToAdd>");
                return true;
            }
            ...
            if (length == 3) {
                ...
                if (!Arrays.asList(listener.getAllGroups(world)).contains(groupToAdd)) {
                    sendMessage(commandSender, um.prefix + "&c&oA group by the name of " + '"' + groupToAdd + '"' + " doesn't exist in world " + '"' + world + '"');
                    return true;
                }
                ...
                if (inher.contains(groupToAdd)) {
                    sendMessage(commandSender, um.prefix + "&c&oGroup " + '"' + group + '"' + " already inherits permissions from group " + '"' + groupToAdd + '"');
                    return true;
                }
                inher.add(groupToAdd);
        ...
        if (length == 0) {
            sendMessage(p, um.prefix + "&c&o/" + commandLabel + " <groupName> <worldName> <groupToAdd>");
            return true;
        }
        ...
        if (length == 3) {
            ...
            if (!Arrays.asList(listener.getAllGroups(world)).contains(groupToAdd)) {
                sendMessage(p, um.prefix + "&c&oA group by the name of " + '"' + groupToAdd + '"' + " doesn't exist in world " + '"' + world + '"');
                return true;
            }
            DataManager dm = new DataManager();
            List<String> worlds = Arrays.asList(um.getWorlds());
            ...
            List<String> inher = groups.getConfig().getStringList(group + ".inheritance");
            if (inher.contains(groupToAdd)) {
                sendMessage(p, um.prefix + "&c&oGroup " + '"' + group + '"' + " already inherits permissions from group " + '"' + groupToAdd + '"');
                return true;
            }
            inher.add(groupToAdd);
            groups.getConfig().set(group + ".inheritance", inher);
